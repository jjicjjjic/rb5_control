...
LowPassFilterVec3 lpf_position(0.05, 2.0);  // dt=0.05초, cutoff=2Hz
LowPassFilterVec3 lpf_velocity(0.05, 2.0);
...
void tcpCommunicationThread() {
    ...
    while (true) {
        ...
        try {
            ...
            // "position"과 "velocity" 항목이 존재할 경우, 전역 데이터를 갱신함.
...
int main() {
    ...
    try {
        ...
        while (true) {
            ...
            {
                lock_guard<mutex> lock(g_dataMutex);
                pos_err = Vector3d(g_position[0], g_position[1], g_position[2]) - p_cur;
            }

            // 가까우면 속도 줄이거나 무시 가능 (원하면 거리 제한 가능)
            if (pos_err.norm() < 1.0) {
