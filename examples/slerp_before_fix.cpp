...
int main() {
    ...
    try {
        ...
        initial_tcp_info[0] = initial_pnt[0];
        initial_tcp_info[1] = initial_pnt[1];
        initial_tcp_info[2] = initial_pnt[2];
        std::cout << "Actual Initial TCP info: " 
                  << initial_tcp_info[0] << ", " 
                  << initial_tcp_info[1] << ", " 
                  << initial_tcp_info[2] << std::endl;
        std::thread initThread(tcpInitialInfoThread);
        initThread.detach();  // 백그라운드에서 계속 실행
