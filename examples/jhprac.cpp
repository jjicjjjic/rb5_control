...
void tcpReceiveThread() {
    ...
    cout << "[TCP] Waiting for GUI client on port " << PORT << endl;
    ...
    while (running.load()) {
        ...
        cout << "[TCP DEBUG] Raw received: " << recv_buffer << endl;

        if (recv_buffer == "s") {
            control_active.store(false);
            running.store(false);
            cout << "[TCP] Stop signal received → 종료 시작.";
            break;
        }
        ...
        try {
            ...
            cout << "[TCP] Target received: x=" << pos[0] << ", y=" << pos[1] << ", z=" << pos[2]
                 << ", roll=" << rad2deg(rpy[0]) << ", pitch=" << rad2deg(rpy[1]) << ", yaw=" << rad2deg(rpy[2]) << endl;
        } catch (const std::exception& e) {
            cerr << "[TCP] Parse error: " << e.what() << endl;
        }
    }
    close(client_fd);
    close(server_fd);
    cout << "[TCP] Server closed.\n";
}
