...
int main() {
    try {
        ...
        robot.set_speed_bar(rc, 0.15);

        robot.flush(rc);

        Vector3d target_position(285.384, -555.692, 300.0);
        ...
        while (true) {
            ...
            // 사용자가 "y"를 입력하면 xyz 좌표 입력
                ...
                // 실시간으로 각속도 출력하는 부분 추가
                vector<double> joint_vel(6);
                ...
                while (true) {
                    ...
                    if (robot_state == podo::RobotState::Moving) {
                        //cout << "Entered new loop";
                        robot.get_system_variable(rc, podo::SystemVariable::SD_J0_VEL, joint_vel[0]);
                        robot.get_system_variable(rc, podo::SystemVariable::SD_J1_VEL, joint_vel[1]);
                        robot.get_system_variable(rc, podo::SystemVariable::SD_J2_VEL, joint_vel[2]);
                        robot.get_system_variable(rc, podo::SystemVariable::SD_J3_VEL, joint_vel[3]);
                        robot.get_system_variable(rc, podo::SystemVariable::SD_J4_VEL, joint_vel[4]);
                        robot.get_system_variable(rc, podo::SystemVariable::SD_J5_VEL, joint_vel[5]);

                        cout << "Joint Velocities: "
                            << joint_vel[0] << ", "
                            << joint_vel[1] << ", "
                            << joint_vel[2] << ", "
                            << joint_vel[3] << ", "
                            << joint_vel[4] << ", "
                            << joint_vel[5] << " [deg/s]" << endl;
