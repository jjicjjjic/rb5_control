...
void executeStaticMove(
    ...
    for (int i = 1; i <= N; ++i) {
        ...
        wp_q[i - 1] = start_q.slerp(s, target_q);
    }

    // 5) Waypoint 순회
    for (int i = 0; i < N; ++i) {
        // 5.1) IK
        Vector3d rpy = wp_q[i].toRotationMatrix()
            .eulerAngles(2, 1, 0).reverse();  // ZYX→[roll,pitch,yaw]
...
int main() {
    ...
    try {
        ...
        Eigen::Vector3d Base_Pose = Eigen::Vector3d(0,400,400);
        Eigen::Vector3d Base_RPY = Eigen::Vector3d(deg2rad(90),deg2rad(0),deg2rad(-90));
        ...
        for (int i = 0;i < 6;++i) {
            ...
            robot.get_system_variable(
                rc, podo::SystemVariable(int(podo::SystemVariable::SD_J0_REF) + i), deg
            );
            q_current[i] = deg * M_PI / 180.0;
        }
        ...
        for (int i = 0;i < 6;++i) {
            q_current[i] = BA[i] * M_PI / 180.0;
        }

        // (2) Planning
        while (true) {
            std::cout
                << "\nEnter target x y z [mm], roll pitch yaw [deg]\n"
                << "(or 'q' to quit): ";

            // 입력 처리
            double x, y, z, rolld, pitchd, yawd;
            if (!(std::cin >> x)) break;
            std::cin >> y >> z >> rolld >> pitchd >> yawd;

            // 목표 pose 생성
            Vector3d target_pos(x, y, z);
            // roll → Rx, pitch → Ry, yaw → Rz (deg→rad)
            Quaterniond qx(AngleAxisd(rolld * M_PI / 180.0, Vector3d::UnitX()));
            Quaterniond qy(AngleAxisd(pitchd * M_PI / 180.0, Vector3d::UnitY()));
            Quaterniond qz(AngleAxisd(yawd * M_PI / 180.0, Vector3d::UnitZ()));
            Quaterniond target_q = qz * qy * qx;  // Z→Y→X 순

            // 경로 실행
            executeStaticMove(rm, robot, rc, q_current, target_pos, target_q);

            std::cout << "→ Reached target. Ready for next.\n";
        }
